    ...
    return (
        <div id={`atfp-${serviceId}-column`} className="atfp-translator-column">
            <div className="atfp-translator-header">
                {ActiveService.Docs &&
                    <a href={ActiveService.Docs} target="_blank"><svg width="9" height="12" viewBox="0 0 9 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {ActiveService.ProviderLink &&
                    <a href={ActiveService.ProviderLink} target="_blank"><svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="atfp-translator-body">
                <strong>{ActiveService.title}</strong><br />
                {ActiveService.Logo &&
                    <a href={ActiveService.ProviderLink} target="_blank"><img src={props.imgFolder+ActiveService.Logo} alt={ActiveService.title} /></a>
                }
                <div className="atfp-translator-btn-wrapper">
                    {ActiveService.ButtonDisabled ?
                        <button disabled={ActiveService.ButtonDisabled} className="atfp-translator-service-btn" id={btnId}>{ActiveService.SettingBtnText}</button> :
                        <button id={btnId} onClick={props.fetchContent} className="atfp-service-btn" data-service={service} data-service-label={ActiveService.ServiceLabel}>{ActiveService.SettingBtnText}</button>}
                    {ActiveService.BetaEnabled && <span className="atfp-translator-beta-btn">Beta</span>}
                </div>
            </div>
            <div className="atfp-translator-footer">
                {ActiveService.ErrorMessage && ActiveService.ErrorMessage}
            </div>
        </div>
    );
}
